...
const DateScheduling = ({ candidatesSubmitted }) => {
  ...
  const handleResultDateChange = (e) => {
    ...
    if (new Date(selectedDate) < minEndTime) {
      setErrorMessage({
        ...errorMessage,
        result: `The result date should be at least 1 hour after the voting date.`,
      });
    } else {
      ...
      {!isScheduled ? (
        ...
        <div className="scheduled-info">
          <div className="info-card">
            ...
            <p className="info-content">{new Date(votingDate).toLocaleString()}</p>
          </div>
          <div className="info-card">
            <h3 className="info-title">Result Date</h3>
            <p className="info-content">{new Date(resultDate).toLocaleString()}</p>
          </div>
        </div>
      )}
    </section>
